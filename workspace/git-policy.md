...
문서 repo는 ticket을 선택으로 두고, bracket에는 문서번호와 버전을 `_`로 연결해 표시한다.
...
- `문서번호`는 문서 번호 또는 문서 식별자를 사용한다.
- `버전`은 해당 문서의 버전을 사용한다.
- bracket 안에는 문서번호와 버전을 `_` 하나로 연결한다.
- 문서번호와 버전에는 영문, 숫자, 점(`.`), 하이픈(`-`)을 사용할 수 있다.
- 문서번호와 버전 내부에는 `_`를 사용하지 않는다.
- Git policy 문서도 다른 문서와 동일하게 `v0.1`, `v0.2` 형식의 문서 버전을 사용한다.
