...
P0 승인 화면은 계정 활성화에 필요한 최소 admin gate로 이 PRD의 범위에 포함한다.
...
### 비밀번호 재설정과 계정 복구 (Password Reset and Account Recovery)
...
### 비밀번호 재설정 및 계정 복구 (Password Reset and Account Recovery)

| ID | 요구사항 |
|---|---|
| APR-FR-070 | P0에서는 앱/웹의 public self-service 비밀번호 재설정 기능을 제공하지 않는다. |
| APR-FR-071 | P0에서는 앱/웹의 public self-service 계정 복구 기능을 제공하지 않는다. |
| APR-FR-072 | P0에서는 관리자용 password reset script만 허용한다. |
| APR-FR-073 | 관리자용 password reset script 실행 시 actor, target account, result, timestamp, reason code를 감사 대상으로 남긴다. |
| APR-FR-074 | 관리자용 password reset script 실행 후 대상 계정의 기존 session과 refresh token은 폐기되어야 한다. |
...
| APR-AC-022 | 사용자가 앱/웹에서 비밀번호 재설정 또는 계정 복구를 찾는다 | P0 화면을 확인한다 | public self-service 비밀번호 재설정/계정 복구 기능은 제공되지 않는다 |
| APR-AC-023 | 관리자용 password reset script가 실행된다 | reset이 성공한다 | 실행 actor, target account, result, timestamp, reason code가 감사 로그에 남고 기존 session/refresh token이 폐기된다 |
| APR-AC-024 | 계정이 `deletion_requested`, `deletion_in_progress`, `deleted` 중 하나다 | 사용자가 삭제 취소 또는 재활성화를 시도한다 | 취소/재활성화는 허용되지 않는다 |
| APR-AC-025 | 삭제 요청 성공 응답이 반환되었다 | auth/gateway 차단 지표를 측정한다 | 차단 반영은 목표 1초 이내, 운영 관측 P99 5초 이내로 기록된다 |
...
- session/device inventory의 `device_id`, `device_name`, `last_ip_hash`를 어떤 방식으로 생성/보관할 것인가?
