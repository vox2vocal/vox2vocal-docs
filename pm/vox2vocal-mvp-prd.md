...
- Goal: 내부 alpha에서 사용자가 본인 음성으로 부른 J-POP 후렴 구간을 업로드하고, 시스템이 이를 보정/생성된 self-voice chorus preview로 들려주는 경험을 1차로 검증한다. 2차로 현재 음정과 목표 음정의 차이를 보여준다. 진성/비성/두성 등 발성/공명 유형 후보는 P0 필수 구현이 아니라 P1 이후 교사/전문가 검토 실험으로 둔다.
...
- Success criteria: baseline과 target은 아직 실제 사용자/처리 데이터가 없어 가정이 필요하다. 내부 alpha에서는 "이 preview가 내 목소리처럼 들린다" 4점 이상을 self-voice preview의 1차 성공 기준으로 두고, chorus-level job completion, pitch matching usefulness, failure reason tagging, safety block rate를 우선 측정한다.
...
- P0 alpha는 Ken Kamikita의 `Mist` 후렴 구간을 첫 target section으로 둔다.
...
- P0 song package 필수 입력값은 title, artist, language, BPM, key, reference audio, source/provenance, usage status, chorus section start/end로 확정한다.

### Section Definition

P0에서 "후렴 구간"은 곡에서 가장 반복적이고 기억에 남는 hook 또는 chorus 부분을 뜻한다. 제품/엔진 관점에서는 음악 이론적 명칭보다 관리자 song package에 등록된 `chorus section start/end timestamp`가 기준이다.

- P0 target song: Ken Kamikita - `Mist`
- P0 target section: 첫 번째 주요 후렴 또는 가장 대표적인 chorus/hook 구간
- 권장 길이: 20-40초. 첫 alpha에서는 30초 안팎을 기본값으로 잡고, 엔진 처리 안정성에 따라 더 짧게 줄일 수 있다.
- 필수 metadata: section label, start timestamp, end timestamp, BPM, key, reference audio source/provenance
- P0 success 판단은 곡 전체가 아니라 이 후렴 구간의 self-voice preview와 pitch feedback을 기준으로 한다.
...
- 장기 제품 목표 입력 길이는 J-POP 또는 우타이테 노래 한 곡 기준이다. 단, P0 내부 alpha에서는 `Mist` 후렴 section 처리로 제한한다.
...
- 한 곡 단위 입력은 처리 시간, 비용, 메모리, 엔진 간 artifact 크기, 오류 복구 난이도를 크게 키울 수 있으므로 P0에서는 후렴 section으로 제한한다.
...
Vox2Vocal P0 MVP는 음악 학습자가 Ken Kamikita의 `Mist` 후렴 구간을 본인 목소리로 부른 녹음을 업로드하면, 시스템이 이를 보정/생성된 self-voice chorus preview로 들려주는 내부 alpha 제품이다. 같은 결과 화면에서 현재 음정과 목표 음정의 차이를 후렴 구간 기준으로 보여준다.

이 PRD의 목적은 "내 목소리로 보정/생성된 후렴 preview를 들어보는 경험"이 실제 음악 학습 동기를 만들 수 있는지 확인하는 것이다. 따라서 P0는 상업 배포, 다중 보이스 모델, 정교한 DAW 편집, 전체 곡 생성, 자동 발성 유형 분석보다 가입, 관리자 곡 등록, 본인 보컬 업로드, 처리 상태 확인, 후렴 self-voice preview 앱 내 재생, 음정 매칭 피드백, 실패 원인 태깅, 본인 음성 권한 차단을 우선한다.
...
그 다음 문제는 원인 파악이다. 학습자는 현재 자신의 음정이 무엇인지, 노래에 필요한 음정과 얼마나 다른지 알기 어렵다. 장기적으로는 진성/비성/두성 같은 발성 또는 공명 유형도 교육적 보조 정보가 될 수 있지만, P0는 이를 자동 판정하지 않는다. P0는 self-voice chorus preview를 1차 가치로 제공하고, 음정 매칭과 실패 원인 태깅을 통해 다음 개발 판단에 필요한 근거를 모으는 데 집중한다.
...
- 학습자가 Ken Kamikita - `Mist` 후렴 구간을 본인 보컬로 업로드한 뒤, 앱 안에서 "내 목소리처럼 들리는" 보정/생성 preview를 들어볼 수 있다.
- 내부 alpha에서 후렴 구간 self-voice preview, pitch 비교, 품질 리포트가 안정적으로 성공하는지 검증한다.
- 학습자가 현재 음정과 목표 음정의 차이를 후렴 구간 기준으로 이해할 수 있다.
- 학습자가 preview를 들은 직후 1-5점 평가를 남기고, 4점 미만이면 실패 원인을 태깅할 수 있다.
- 제품/엔진 팀이 후렴 preview 실패 원인, pitch mismatch, artifact, 처리 stage 실패를 구분해서 볼 수 있다.
...
- 관리자 수동 song package 등록: Ken Kamikita - `Mist`를 기준으로 title, artist, language, BPM, key, reference audio, source/provenance, usage status, 후렴 section start/end를 필수로 등록한다.
...
- 관리자 song package 등록: pitch target 추출과 비교 분석 목적의 원곡/reference audio, metadata, BPM, key, chorus section 정보를 등록. P0에서는 Ken Kamikita - `Mist` 수동 등록과 후렴 section을 기준으로 하며 provider 자동화와 lyrics sync는 P1 실험 범위로 둔다.
- 학습자 곡 선택: 학습자는 관리자 등록 곡을 선택하고 본인 보컬 연습 파일만 업로드
- 입력 길이 목표: `Mist` 후렴 section 기준 20-40초. P0에서는 전체 곡 입력을 요구하지 않는다.
- 입력 metadata: 곡/작업명, 언어, BPM, key, 원곡/목표곡 정보, chorus section start/end
- Audio Ingest: 표준 PCM/WAV 변환, mono 변환, 무음 탐지, 발화 구간 timestamp 생성
- Voice Pitch: 후렴 구간 F0 추출, confidence filtering, MIDI note 변환, target pitch 비교 JSON
- Target Pitch Mapping: 관리자 reference audio 또는 엔진 추정 note sequence를 사용한 후렴 target pitch/note 생성
- Self-voice Preview: 사용자 본인 음성 기반 후렴 self-voice preview 생성
...
- As a music learner, I want to upload my own `Mist` chorus practice recording so that I can hear a corrected/generated chorus preview in my own voice.
- As a music learner, I want to select the admin-registered `Mist` chorus package so that the analysis uses the intended target section.
- As a music learner, I want to compare my current pitch with the target pitch for the chorus so that I know whether I am singing the right notes.
...
- As a product/engineering team member, I want evaluation metrics, failure reason tags, and engine artifacts so that I can debug quality regressions in the P0 chorus preview loop.
...
- The selected P0 song package must be Ken Kamikita - `Mist` and provide default language, BPM, key, reference audio, and chorus section metadata. Lyrics are optional in P0.
...
- The system must record original duration and whether the upload matches the configured chorus section length tolerance.
- If the uploaded recording is longer than the P0 chorus input limit, the system must ask the user to trim or select the expected chorus section instead of silently processing the full song.
...
### FR-007 Chorus Pitch Extraction

- The system must extract F0 frames for the uploaded chorus practice recording.
...
- The system must compare detected pitch against target notes for the configured `Mist` chorus section.
...
### FR-009 Self-voice Chorus Preview

- The system must generate a corrected/generated self-voice chorus preview using the user's own voice only.
- The system must render the result for app-only playback.
- The render output must include clipping and loudness metadata.
- The preview must be clearly labeled as section-limited chorus output.
...
- Completed jobs must show that the result is a section-limited chorus preview.
...
- P0 jobs must expose duration, per-stage processing time, failed stage, and chorus section details for technical review.
...
- Given an admin creates the Ken Kamikita - `Mist` P0 song package, when title, artist, language, BPM, key, reference audio, source/provenance, usage status, or chorus section start/end is missing, then the package cannot be published to learners.
- Given an admin creates the `Mist` P0 song package with required metadata, reference audio, source/provenance, usage status, and chorus section, when validation passes, then learners can select that chorus package in the alpha app.
- Given an authenticated user uploads a supported `wav` or `mp3` chorus practice recording, when the upload completes, then the system creates a P0 chorus preview job and an `audio_asset_id`.
...
- Given an uploaded recording is longer than the configured P0 chorus tolerance, when validation runs, then the user is asked to trim or select the expected chorus section instead of silently processing the full song.
...
- Given a valid chorus practice recording with required metadata, when preview generation completes, then a corrected/generated self-voice chorus preview is available for app-only playback.
...
- Given target notes are available from reference audio analysis or engine-derived note sequence, when pitch analysis completes, then the user can see current pitch, target pitch, and sharp/flat/on-target status for the chorus section.
- Given reference audio analysis and engine-derived note sequence conflict, when confidence is low or disagreement is high, then the system labels the affected chorus range as low confidence or review needed instead of forcing a target note.
...
- Given a P0 chorus job completes or fails, when internal reviewers inspect the artifacts, then duration, per-stage timing, failed stage, and chorus section details are available.
...
- Build can start when the team registers Ken Kamikita - `Mist`, defines the chorus target section start/end, and confirms required song package metadata.
...
- Section technical feasibility
  - Baseline: Unknown.
  - Target: Internal alpha produces enough processing data to decide whether the `Mist` chorus preview loop should continue, be shortened, or be redesigned.
...
- Metric risk: There is no real baseline for chorus-level completion, self-voice preview usefulness, pitch matching usefulness, failure reason distribution, or activation.
- UX risk: Users may expect polished full-song output, while alpha output may be section-limited or have quality caveats.

## Dependencies

- App: upload screen, job status screen, chorus result screen, app-only result playback UI, preview rating UI, failure reason tagging UI, token/session integration
...
- Engines: audio ingest, voice pitch, target pitch mapping, self-voice chorus preview, preview evaluation, safety rights
...
- Music education expertise: chorus section selection, pitch feedback interpretation, low-confidence cases, and learner-facing explanation copy

## Open Questions

### Product And User

- Ken Kamikita - `Mist`의 P0 후렴 start/end timestamp는 몇 초로 확정할 것인가?
...
- P0 후렴 preview에서 허용 가능한 time-to-preview 관찰 기준은 몇 분으로 볼 것인가?
...
- P0 후렴 preview의 최소 viable engine path는 mock, partial pipeline, real synthesis 중 무엇으로 시작할 것인가?
...
- MVP의 장기 제품 목표는 J-POP/우타이테 노래 한 곡 기준이지만, P0 내부 alpha는 `Mist` 후렴 section으로 제한한다.
- 주요 사용자는 음악 학습자와 음악 교육자다.
- 내부 alpha 사용자 규모는 학습자 10명과 교육자 2명이다.
- P0 alpha는 Ken Kamikita - `Mist` 후렴 section, 관리자 수동 song package 등록, pitch-first processing, section-first preview를 기준으로 build한다.
...
- 현재 구현 상태를 기준으로 auth 연동, upload/job API, chorus result UI, canonical P0 job state owner, minimal engine path가 주요 신규 개발 범위다.
- PRD 승인 전에는 후렴 start/end timestamp, self-voice preview 평가 문항 UI, failure reason tag 목록, song package 필수 metadata, 저장/삭제 정책, P0 job state owner를 반드시 확정해야 한다.
