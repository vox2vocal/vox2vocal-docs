...
- Success criteria: baseline과 target은 아직 실제 사용자/처리 데이터가 없어 가정이 필요하다. 내부 alpha에서는 self-voice preview usefulness, song-level job completion, pitch matching usefulness, vocal-mode insight usefulness, safety block rate를 우선 측정한다.
...
- Vocal Mode Insight: 진성/비성/두성 등 발성/공명 유형 후보, confidence, 근거 구간 표시
- Safety Rights: 사용자 본인 업로드 보이스만 허용, conversion audit log
- 작업 상태 화면: queued, processing, completed, failed
- 결과 화면: self-voice preview 재생, 다운로드, 현재/목표 음정 비교, 발성/공명 유형 후보, 구간별 학습 리포트, 실패 사유
...
- Completed jobs must show current pitch vs target pitch, pitch match score or deviation, low-confidence pitch sections, vocal mode candidates, section-level learning feedback, download action if allowed, and basic quality report.
...
### FR-013 Preview Quality Evaluation
...
- The system must not mark a job as fully successful if the learning report succeeds but the required self-voice preview fails.

### FR-014 Pitch And Vocal Mode Feedback
...
### FR-015 Evaluation And Observability
...
- Given a valid practice recording with required metadata, when the pipeline completes, then a corrected/generated self-voice song preview is available for playback or the section-limited preview state is clearly shown.
...
- Given target notes are available, when pitch analysis completes, then the user can see current pitch, target pitch, and sharp/flat/on-target status by section.
- Given vocal mode analysis has enough confidence, when the user opens the result screen, then 진성/비성/두성 or other vocal mode candidates are shown with confidence and caveats.
...
- Self-voice preview usefulness
  - Baseline: Unknown.
  - Target: At least 50% of learner/teacher alpha testers rate the self-voice preview as useful enough to motivate another practice attempt.
...
- Pitch matching usefulness
  ...
  - Guardrail: Low-confidence pitch sections must be clearly labeled and excluded from overconfident scores.
- Vocal mode insight usefulness
  - Baseline: Unknown.
  - Target: At least 40% of learner/teacher alpha testers say 진성/비성/두성 candidate feedback is understandable and worth keeping.
  - Guardrail: Vocal mode labels must include confidence or insufficient-confidence states and must not be presented as definitive diagnosis.
- Quality diagnostics
  - Baseline: Unknown.
...
- self-voice preview의 alpha 성공 기준은 무엇인가: 내 목소리처럼 들림, 노래처럼 들림, 음정 보정 체감, 다시 연습하고 싶음 중 무엇을 primary rating으로 볼 것인가?
...
- self-voice preview 품질을 어떤 기준으로 평가할 것인가: 본인 음색 유사도, 음정 안정성, 자연스러움, 노래 완성도, artifact 적음 중 무엇을 우선할 것인가?
- pitch matching은 원곡 음원, MIDI/악보, 사용자가 입력한 key/BPM, 또는 엔진 추정 note sequence 중 무엇을 target 기준으로 삼을 것인가?
- 진성/비성/두성 라벨은 어떤 음악교육 정의를 따를 것인가?
- 발성/공명 유형 분석은 user-facing으로 바로 제공할 것인가, 내부 alpha에서는 teacher-facing 검토 정보로 먼저 둘 것인가?
...
- self-voice preview가 실패하고 음정/발성 리포트만 성공하면 alpha success로 볼 것인가, partial success로 볼 것인가?
...
- 발성/공명 유형 분석은 내부 alpha에서 확정 판정이 아니라 confidence가 있는 후보 정보로 제공한다.
- 출시 형태는 내부 alpha다.
...
- PRD 승인 전에는 self-voice preview 품질 기준, pitch target source, vocal mode 라벨 정의, full-song 처리 방식, success metric target을 반드시 확정해야 한다.
